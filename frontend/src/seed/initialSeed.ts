  ...
  workouts: [
    ...
    },
    {
      ...
      exercises: [
        ...
        { name: 'Rosca W', setsReps: '4x10-12', notes: 'Bíceps foco total', gifUrl: INITIAL_EXERCISES[3].gifUrl },
...
export function ensureInitialClientSeed() {
  if (!localStorage.getItem('fitpulse_client_seeded_v1')) {
    localStorage.setItem('fitpulse_gyms', JSON.stringify(INITIAL_GYMS));
    localStorage.setItem('fitpulse_users', JSON.stringify(INITIAL_USERS));
    localStorage.setItem('fitpulse_exercises', JSON.stringify(INITIAL_EXERCISES));
    localStorage.setItem('fitpulse_schedules', JSON.stringify([INITIAL_SCHEDULE]));
    localStorage.setItem('fitpulse_payments', JSON.stringify(INITIAL_PAYMENTS));
    localStorage.setItem('fitpulse_client_seeded_v1', 'true');
  }
}
